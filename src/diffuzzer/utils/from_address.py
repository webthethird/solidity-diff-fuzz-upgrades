...
def get_deployed_contract(contract_data: ContractData, implementation: str, slither_provider: NetworkSlitherProvider, network_info: NetworkInfoProvider) -> tuple[Contract, Slither|None, Contract|None]:
    ...
    if contract.is_upgradeable_proxy:
        if implementation == "":
            implementation, contract_data = network_info.get_proxy_implementation(contract, contract_data)
...
def get_contract_data_from_address(address: str, implementation: str, slither_provider: NetworkSlitherProvider, network_info: NetworkInfoProvider, suffix: str = "") -> ContractData:
    ...
    try:
        contract_data["slither"] = slither_provider.get_slither_from_address(contract_data["address"])
    ...
    if contract_data["valid_data"]:
        contract, impl_slither, impl_contract = get_deployed_contract(contract_data, implementation, slither_provider, network_info)
...
def get_contracts_from_comma_separated_string(addresses_string: str, slither_provider: NetworkSlitherProvider, network_info: NetworkInfoProvider) -> tuple[list[ContractData], list[str], dict]:

    results = []

    [addresses, implementations] = addresses_from_comma_separated_string(addresses_string)
    for a in addresses:
        data = get_contract_data_from_address(a, implementations.get(a, ""), slither_provider, network_info)
