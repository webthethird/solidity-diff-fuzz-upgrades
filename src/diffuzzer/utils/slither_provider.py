...
class SlitherProvider:
    ...
    def _get_slither_from_cache(self, address: str) -> Slither|None:
        ...
        if os.path.exists(self._cache_path + self._cache_filename):
            CryticPrint.print_success(f"    * Contract {self._network_prefix}-{address} found in cache.")
    ...
    def _save_slither_to_cache(self) -> None:
        ...
        save_to_zip([self._slither_object.crytic_compile], self._cache_path + self._cache_filename)
        
        CryticPrint.print_success(f"      * Contract {self._cache_filename[:-4]} obtained and cached.")
...
class NetworkSlitherProvider(SlitherProvider):   
    ...
    def get_slither_from_address(self, address: str) -> Slither:
        ...
        if s is not None:
            ...
            s = Slither(f"{self._network_prefix}:{address}", bscan_api_key=self._api_key)
            self._slither_object = s
