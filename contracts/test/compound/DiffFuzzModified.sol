...
import { ComptrollerHarness as Comptroller_V1 } from "../../implementation/compound/compound-0.8.10/ComptrollerV1.sol";
import { ComptrollerHarness as Comptroller_V2 } from "../../implementation/compound/compound-0.8.10/ComptrollerV2.sol";
...
//
   function Comptroller_compSupplyState(address a) public {
       assert(comptrollerV1.compSupplyState(a).index == comptrollerV2.compSupplyState(a).index);
   }

   function Comptroller_compBorrowState(address a) public {
       assert(comptrollerV1.compBorrowState(a).index == comptrollerV2.compBorrowState(a).index);
   }

    function Comptroller_compSupplierIndex(uint a, address b) public {
        uint marketIndex = a % IComptrollerV1(address(unitrollerV1)).getAllMarkets().length;
        address a1 = IComptrollerV1(address(unitrollerV1)).getAllMarkets()[marketIndex];
        address a2 = IComptrollerV2(address(unitrollerV2)).getAllMarkets()[marketIndex];
        assert(IComptrollerV1(address(unitrollerV1)).compSupplierIndex(a1, b) == IComptrollerV2(address(unitrollerV2)).compSupplierIndex(a2, b));
    }

    function Comptroller_compBorrowerIndex(uint a, address b) public {
        uint marketIndex = a % IComptrollerV1(address(unitrollerV1)).getAllMarkets().length;
        address a1 = IComptrollerV1(address(unitrollerV1)).getAllMarkets()[marketIndex];
        address a2 = IComptrollerV2(address(unitrollerV2)).getAllMarkets()[marketIndex];
        assert(IComptrollerV1(address(unitrollerV1)).compBorrowerIndex(a1, b) == IComptrollerV2(address(unitrollerV2)).compBorrowerIndex(a2, b));
    }
...
//    }
//
    function CErc20_balanceOf() public virtual {
        hevm.prank(msg.sender);
        (bool success2, bytes memory output2) = address(cErc20V2).call(
            abi.encodeWithSelector(
                cErc20V2.balanceOf.selector, msg.sender
            )
        );
        hevm.prank(msg.sender);
        (bool success1, bytes memory output1) = address(cErc20V1).call(
            abi.encodeWithSelector(
                cErc20V1.balanceOf.selector, msg.sender
            )
        );
        assert(success1 == success2);
        assert((!success1 && !success2) || keccak256(output1) == keccak256(output2));
...
//    }
    ...
    function Comp_balanceOf() public virtual {
        ...
        (bool success2, bytes memory output2) = address(compV2).call(
            abi.encodeWithSelector(
                compV2.balanceOf.selector, msg.sender
            )
        );
        hevm.prank(msg.sender);
        (bool success1, bytes memory output1) = address(compV1).call(
            abi.encodeWithSelector(
                compV1.balanceOf.selector, msg.sender
            )
        );
        assert(success1 == success2); 
        assert((!success1 && !success2) || keccak256(output1) == keccak256(output2));
