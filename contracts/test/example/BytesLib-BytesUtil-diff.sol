...
contract Test {
    ...
    function test_BytesLib_BytesUtil_diff_slice(
        bytes calldata input,
        uint256 start,
        uint256 length
    ) external {
        // Using calls to ensure reverts from either lib won't make forge fuzzing consider this a failed test.
        (bool successA, bytes memory outputA) = address(bytesLib).call(
            abi.encodeWithSelector(
                bytesLib.slice.selector,
                input,
                start,
                length
            )
        );
        (bool successB, bytes memory outputB) = address(bytesUtil).call(
            abi.encodeWithSelector(
                bytesUtil.slice.selector,
                input,
                start,
                length
            )
        ...
        assert(
            (!successA && !successB) || keccak256(outputA) == keccak256(outputB)
        );
    }
}
