...
def test_bad_poa_init() -> None:
    ...
    try:
        ...
        assert str(err) == "Got ExtraDataLengthError when getting block latest. " \
...
def test_bad_block_init() -> None:
    ...
    try:
        ...
        assert str(err) == '"final" is not a valid block identifier. Use "latest", "earliest", ' \
...
def test_missing_token_holders() -> None:
    ...
    try:
        ...
        assert str(err) == f"Contract at {contract_addr} doesn't appear to be a token. " \
